...
const translations = {
  ...
  de: {
    ...
    table: {
      ...
      search: {
        placeholder: 'Suche nach Kundenname, Reisepassnummer oder E-Mail...'
      },
      pagination: {
        previous: 'Zurück',
        next: 'Weiter',
...
function CustomersList() {
  ...
  const handleResizeStart = (field, e) => {
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      const currentX = moveEvent.clientX || (e.touches && e.touches[0]?.clientX) || 0
      if (!currentX) return
  ...
  const handleResizeDoubleClick = (field, e) => {
    ...
    customers.forEach(customer => {
      const value = getCellValue(customer, field)
      if (value) {
  ...
  // Filter customers by date range (based on last booking date)
  const filterByDate = (customers, filterType) => {
    ...
    return customers.filter(customer => {
      ...
      const lastBookingDate = new Date(customer.last_booking_date)
      lastBookingDate.setHours(0, 0, 0, 0)
      
      if (range.startDate && lastBookingDate < range.startDate) return false
      if (range.endDate) {
        const endDate = new Date(range.endDate)
        endDate.setHours(23, 59, 59, 999)
        if (lastBookingDate > endDate) return false
      }
  ...
  // Group customers by month (based on last booking date)
  const groupByMonth = (customers) => {
    const groups = {}
    
    customers.forEach(customer => {
      if (!customer.last_booking_date) return
      
      const date = new Date(customer.last_booking_date)
  ...
  useEffect(() => {
    if (customers.length > 0 && expandedGroups.size === 0) {
      const now = new Date()
  ...
  const getCellValue = (customer, field, rowIndex = null) => {
    if (field === 'row_number') {
    ...
    switch (field) {
      ...
      case 'total_bookings':
        return String(customer.total_bookings || 0)
      case 'total_spent':
        return formatCurrency(customer.total_spent)
      case 'last_booking':
        return customer.last_booking_date ? formatDateTime(customer.last_booking_date) : '-'
      case 'status':
  ...
  // Fetch and process customers from mock store
  const fetchCustomers = () => {
    try {
      setLoading(true)
      setError(null)

      const bookings = store.mainTable.getAll()
        .slice()
        .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))

      // Group bookings by customer (using passport_number as unique identifier)
      const customerMap = {}
      ...
      bookings.forEach(booking => {
        ...
        if (!passportNumber && !fullName) return // Skip if no identifier
        ...
        customerMap[customerKey].total_bookings++
        const amount = parseFloat(booking.total_amount_due) || 0
        customerMap[customerKey].total_spent += amount
        
        const bookingDate = booking.created_at ? new Date(booking.created_at) : null
        if (bookingDate && (!customerMap[customerKey].last_booking_date || bookingDate > new Date(customerMap[customerKey].last_booking_date))) {
          customerMap[customerKey].last_booking_date = booking.created_at
        }
        
        customerMap[customerKey].bookings.push(booking)
      })
      
      // Convert to array and sort by last booking date (most recent first)
      const customersArray = Object.values(customerMap).sort((a, b) => {
  ...
  useEffect(() => {
    fetchCustomers()
    const unsubscribe = store.subscribe(fetchCustomers)
    return unsubscribe
  ...
  const getColumnLabel = (field) => {
    const labelMap = {
      ...
      customer_name: t.table.columns.customerName,
      passport_number: t.table.columns.passportNumber,
      nationality: t.table.columns.nationality,
      total_bookings: t.table.columns.totalBookings,
      total_spent: t.table.columns.totalSpent,
      last_booking: t.table.columns.lastBooking,
  ...
  // Apply filters
  const dateFilteredCustomers = filterByDate(customers, dateFilter)
  const filteredCustomers = searchTerm.trim() === '' 
    ...
    : dateFilteredCustomers.filter(customer => {
        ...
        const passportNumber = (customer.passport_number || '').toLowerCase()
  ...
  const groupedCustomers = groupByMonth(filteredCustomers)
  ...
  const handleViewBookings = (customer) => {
    // Filter bookings by this customer and navigate to bookings page
    const passportNumber = customer.passport_number
    if (passportNumber && passportNumber !== '-') {
      window.open(`/bookings?passport=${encodeURIComponent(passportNumber)}`, '_blank')
    }
  }
  ...
  return (
    <div className="page-layout">
      ...
      {/* Main Content */}
      <main className="main-content customers-page">
          ...
          {!loading && !error && customers.length === 0 && (
            <div className="requests-empty-state">
              <div className="empty-state-content">
                ...
                <p className="empty-state-description">Customers will appear here once bookings are created</p>
              </div>
            </div>
          )}
            ...
            <div className="invoice-summary-cards" style={{ 
              ...
              <div className="summary-card" style={{
                ...
                <div style={{ fontSize: '0.875rem', color: 'hsl(210, 20%, 70%)' }}>
                  {customerStats.totalBookings} total bookings
                </div>
              </div>
              ...
              <div className="summary-card" style={{
                ...
                <div style={{ fontSize: '0.875rem', color: 'hsl(210, 20%, 70%)' }}>
                  {customerStats.total > 0 ? Math.round((customerStats.active / customerStats.total) * 100) : 0}% of total
                </div>
              </div>
              ...
              <div className="summary-card" style={{
                ...
                <div style={{ fontSize: '2rem', fontWeight: '700', color: 'hsl(280, 70%, 60%)', marginBottom: '0.25rem' }}>
                ...
                <div className="summary-card" style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                    <div style={{
                      ...
                      height: '40px',
                      borderRadius: '0.5rem',
                      background: 'hsla(210, 20%, 50%, 0.1)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center'
                    }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="hsl(210, 20%, 50%)" strokeWidth="2">
                  ...
                  <div style={{ fontSize: '2rem', fontWeight: '700', color: 'hsl(210, 20%, 50%)', marginBottom: '0.25rem' }}>
                    {customerStats.inactive}
                  </div>
                  <div style={{ fontSize: '0.875rem', color: 'hsl(210, 20%, 70%)' }}>
                    {customerStats.total > 0 ? Math.round((customerStats.inactive / customerStats.total) * 100) : 0}% of total
                  </div>
                </div>
              )}
            </div>
                  ...
                  {groupedCustomers.length === 0 ? (
                    <tr>
                      <td colSpan={columnOrder.length} style={{ textAlign: 'center', padding: '2rem' }}>
                        {dateFilter || searchTerm.trim() ? 'No matching records found.' : t.table.noCustomers}
                      </td>
                    </tr>
                  ) : (
                    <>
                      {groupedCustomers.map((group) => {
                        const flatCustomers = group.customers
                        return (
                          <React.Fragment key={group.key}>
                            ...
                            {(!groupedCustomers.length > 1 || expandedGroups.has(group.key)) && flatCustomers.map((customer, index) => {
                              const globalIndex = groupedCustomers.reduce((acc, g, gIdx) => {
                                if (gIdx < groupedCustomers.indexOf(group)) {
                                  return acc + g.customers.length
                                }
                                return acc
                              }, 0) + index
                                    ...
                                    return (
                                      ...
                                      >
                                        {field === 'actions' ? (
                                          ...
                                          <div className="excel-cell">
                                            <span className={`status-badge status-${getCustomerStatus(customer)}`}>
                                              {t.table.status[getCustomerStatus(customer)]}
                                            </span>
                                          </div>
                                        ) : (
                                          <div className="excel-cell excel-cell-readonly">
                                            {getCellValue(customer, field, globalIndex)}
                                          </div>
                                        )}
                                      </td>
                                    )
